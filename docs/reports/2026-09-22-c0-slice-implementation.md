...
23 evidence-backed PASS cases, zero FAIL/BLOCKED cases, and one explicit
NOT_RUN case. This is not a production-readiness or deployment approval.
...
| C0.4 golden qualification | `tests/fixtures/c0-qualification/registry.json`, `result-manifest.json`, `scripts/check-c0-qualification.mjs`, and the process-loss replay integration; the checker rejects missing case coverage, unsupported statuses, PASS without evidence, and unresolved FAIL/BLOCKED cases. Static golden hashes are not independently regenerated |
...
| Current `npm test` suite (Node 24.19.0) | PASS — 224 passed, 2 skipped across 30 files; security suite 11/11 passed |
...
| C0 lost-response replay | PASS — stdio subprocess loses the successful submit response after dispatch; restart replay returns the same decision and scope, and preserves one batch plus four Stage 9-12 evidence rows/cursors |
| C0 registry/checker | PASS — 23 PASS, 1 NOT_RUN, 0 FAIL/BLOCKED |
...
- Node 22 is `NOT_RUN` locally; the existing CI matrix remains the required
  cross-runtime check.
- Tier-4 worker/GenesisBlockDB readback, hardware/power-loss, backup/restore,
...
| 0.2.1 | 2026-09-25 | beta | Added process-loss replay qualification evidence; Tier-4 remains NOT_RUN | working-tree | RWANG |
