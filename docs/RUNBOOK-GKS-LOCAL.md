...
The package declares Node `>=22`. `.github/workflows/test.yml` runs `npm ci` and
`npm test` on Node 22 and 24 (ubuntu-latest) for every pull request and push to
`main`; the external MSP suite skips there because `MSP_REPO_ROOT` is unset.
`npm ci` must report no unresolved security advisories before release
consideration.

The external MSP acceptance profile is separate from GKS CI. It uses the
explicit Node `v24.18.x` runtime documented by the MSP runbook and rebuilds the
MSP `better-sqlite3` binding against matching headers before running the real
service-chain tests. Record the runtime, native build target, MSP commit and
GKS commit with every compatibility result; do not treat a different local Node
...
|---|---|---|---|---|---|
| 0.2.3b | 2026-09-20 | beta | Reconciled the runbook with the current MSP Node 24.18 native-binding acceptance profile and separated it from GKS's Node 22/24 CI lanes. | working-tree | RWANG |
| 0.2.2b | 2026-09-11 | beta | Node floor raised to `>=22` for the GKS package and named the CI workflow. | working-tree | Claude Opus 5 |
